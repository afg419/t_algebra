...
RSpec.describe TAlgebra::Monad::Parser do
  ...
  context "run syntax" do
    it "parses an API result to a HallOfFamer using run" do
      ...
      hall_of_famer = TAlgebra::Monad::Parser.run do
        # validate presence with `.fetch!` and type with `#is_a?``
        first = _pick { fetch!(api_result, :firstName).is_a?(String) }
        last = _pick { fetch!(api_result, :lastName).is_a?(String) }
        ...
        born = _pick { fetch!(api_result, :born).fmap { |x| Date.parse(x) } }
        died = _pick { fetch(api_result, :died).fmap { |x| Date.parse(x) } }
        ...
        team_hash = _pick { fetch!(api_result, :team).is_a?(Hash) }
        team = _pick do
